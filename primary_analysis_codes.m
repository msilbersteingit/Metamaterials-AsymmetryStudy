...
% Initialize generative design values
numDes = 10;
numextra = 2;
...
%% INITIALIZE SYMMETRY-SPECIFIC VALUES
% Select symmetry type
symmtype = 1;

% Initialize symmetry subset
symmsubCABasket = [];
symmsubCBasket = [];
symmsubvfBasket = [];
symmsubCaxBasket = [];
...
for smprob = 0.2:0.2:1
    ...
    for i = 1:1:6
        ...
        while count == err_count
            try
                if symmtype ~= 0
                    tinynewbatchCAs = gen_Des_func_V3(sidenum,sel,4,smprob,0,numextra,symmtype);
                else
                    tinynewbatchCAs = gen_Des_func_V2(sidenum,sel,3,smprob,0,numextra);
                end
            catch ME
        ...
        mednewbatchCAs = [mednewbatchCAs,tinynewbatchCAs];
    end
    newbatchCAs = [newbatchCAs,mednewbatchCAs];
end
newbatchCs = [];
close all
...
while count == err_count
    try
        tinynewbatchCAs = gen_Des_func_V3(sidenum,sel,10,1,0,numextra,symmtype);
    catch ME
        err_count = err_count + 1;
    end
    count = count + 1;
end
newbatchCAs = [newbatchCAs,tinynewbatchCAs];
newbatchCs = [];
close all

%% Ensure no new designs are repetitions, and also check for repeatability

%if ~isempty(symmsubCABasket)
    % Isolate unique CAs
    %usedCAs = symmsubCABasket; 
    %usedCAs = {};
    uCAc = length(usedCAs);
    tobeusedCAs = {};
    for j = 1:1:length(usedCAs)
        CA = cell2mat(usedCAs(j));
        SortedCA = sortrows((sort(CA'))');
        usedCAs(j) = {SortedCA};
    end

    % Loop through all designs
    for v = 1:1:length(newbatchCAs)
        % Initialize values
        CA = cell2mat(newbatchCAs(v));
    ...
    newbatchCAs = tobeusedCAs;
%end

%% Model C-matrices of incremental batch
for cac = 1:1:size(newbatchCAs,2)
    CA = cell2mat(newbatchCAs(cac));
    ...
    % Calculate C, volFrac
    C = calcCTruss(csPropsA,csPropsB,edgelog,NP,...
...
%% Postprocess incremental batch, find vf values

% Postprocessing
newbatchCs = newbatchCs.*100;
C11Basket = []; C22Basket = [];
goodCABasket = {};
goodCBasket = [];
counter = 1;
for k = 1:1:size(newbatchCs,3)
    Ctemp = newbatchCs(:,:,k); 
    Stemp = inv(Ctemp);
    ...
    tinyCA = cell2mat(newbatchCAs(k));
    if (~isnan(vxy) && ~isnan(vyx))
        if (abs(vxy) <= 1) && (abs(vyx) <= 1)
            C11Basket = [C11Basket,Ctemp(1,1)];
            C22Basket = [C22Basket,Ctemp(2,2)];
            goodCBasket = cat(3,goodCBasket,Ctemp);
            goodCABasket(counter) = {tinyCA}; 
            counter = counter + 1;
        end
    end
end
goodCABasket = goodCABasket(~cellfun('isempty',goodCABasket));

% Volume fraction calculation
newCaxBasket = [];
newCaxBasket = [C11Basket,C22Basket]./E; 
newdblgoodCABasket = [goodCABasket,goodCABasket];
newvfBasket = []; 
for cac = 1:1:size(newdblgoodCABasket,2)
    CA = cell2mat(newdblgoodCABasket(cac));
    vf = calcVF_NxN_feasOnly(CA,r,sel,sidenum);
    if isnan(vf)
        disp(CA);
        break
    end
    newvfBasket = [newvfBasket,vf];
end

%% Perform 2-sample K-S tests to gauge evolution of design set
[bin1, pval1] = kstest2([symmsubCaxBasket,newCaxBasket],symmsubCaxBasket);
[bin2, pval2] = kstest2([symmsubvfBasket,newvfBasket],symmsubvfBasket);
disp(pval1);
disp(pval2);

%% Integrate new designs into existing set
symmsubCaxBasket = [symmsubCaxBasket,newCaxBasket];
symmsubvfBasket = [symmsubvfBasket,newvfBasket];
symmsubCABasket = [symmsubCABasket,newdblgoodCABasket];
symmsubCBasket = cat(3,symmsubCBasket,goodCBasket);
for c = 1:1:size(goodCBasket,3)
    Ctemp = goodCBasket(:,:,c); 
    flipC = [Ctemp(2,2),Ctemp(2,1),Ctemp(3,1);
             Ctemp(1,2),Ctemp(1,1),Ctemp(3,2);
             Ctemp(1,3),Ctemp(2,3),Ctemp(3,3)];
    symmsubCBasket = cat(3,symmsubCBasket,flipC);
end

%% Save output-- rot symm
symmCABasket = symmsubCABasket;
symmCBasket = symmsubCBasket;
symmCaxBasket = symmsubCaxBasket;
symmvfBasket = symmsubvfBasket;
save('symm_new_5x5.mat','symmCABasket','symmCBasket','symmCaxBasket','symmvfBasket');
...
load(strcat('USNCTAM_',num2str(rnum),'_3x3_Dataonly'));
CABasket = {};
CBasket = [];
for j = 1:1:3
    newCA = eval(strcat('CABasket_done_',num2str(j)));
    newC = eval(strcat('CBasket_done_',num2str(j)));
...
asymmC66Basket = []; fsymmC66Basket = []; symmC66Basket = [];
for k = 1:1:size(asymmCBasket,3)
    Ctemp = asymmCBasket(:,:,k); 
    asymmC66Basket = [asymmC66Basket,Ctemp(3,3)];
end
for k = 1:1:size(fsymmCBasket,3)
    Ctemp = fsymmCBasket(:,:,k); 
    fsymmC66Basket = [fsymmC66Basket,Ctemp(3,3)];
end
for k = 1:1:size(symmCBasket,3)
    Ctemp = symmCBasket(:,:,k); 
    symmC66Basket = [symmC66Basket,Ctemp(3,3)];
end

reldens = linspace(0,0.8,100);
K = E/(3*(1-(2*v_c)));
G = E/(2*(1+v_c));
GHS = (((9.*K)+(8.*G)).*reldens)./((20.*G)+(15.*K)+(6.*((K)+...
(2.*G)).*(1-reldens)));
j1 = plot(modasymmvfBasket,asymmC66Basket./E,'Marker','+',...
'MarkerSize',4,'LineStyle','none','Color',[0.3010 0.7450 0.9330]);
hold on
h1 = plot(fsymmvfBasket,fsymmC66Basket./E,'Marker','o',...
'MarkerSize',4,'LineStyle','none','Color',[0.4660 0.6740 0.1880]);
if sidenum == 5
    g1 = plot([symmvfBasket],[modsymmC66Basket./E],'Marker','*',...
    'MarkerSize',6,'LineStyle','none','Color',[0.4940 0.1840 0.5560]);
elseif sidenum == 3
    g1 = plot([symmvfBasket],[modsymmC66Basket./E],'Marker','*',...
    'MarkerSize',6,'LineStyle','none','Color',[0.4940 0.1840 0.5560]);
end
%hold on
plot(reldens,GHS,'b-','LineWidth',1);
if sidenum == 5
    plot(0.6148.*ones(1,100),0:0.01:0.99,'k--','LineWidth',1);
elseif sidenum == 3
    plot(0.34669.*ones(1,100),0:0.01:0.99,'k--','LineWidth',1);
end
grid on
xlabel('Volume Fraction');
ylabel('Shear Stiffness');
legend('Asymmetric Designs','Mirror Symmetric Designs','Double-Mirror Symmetric Designs','Upper H-S Bound','Maximum Volume Fraction','Location','northeast');

j1.ButtonDownFcn = {@plotDesignFcn2,2,asymmCABasket,sel,sidenum,NC}; 
h1.ButtonDownFcn = {@plotDesignFcn2,2,fsymmCABasket,sel,sidenum,NC};
g1.ButtonDownFcn = {@plotDesignFcn2,2,symmCABasket,sel,sidenum,NC}; 

axis([0 0.7 0 0.1]);
set(gca,'fontsize', 13)

% Develop regressions
asfit = polyfit(asymmvfBasket,(asymmC66Basket./E),1);          
asyfit = polyval(asfit,asymmvfBasket);          
asSStot = sum(((asymmC66Basket./E)-mean((asymmC66Basket./E))).^2);                   
asSSres = sum(((asymmC66Basket./E)-asyfit).^2);                       
asRsq = 1-asSSres/asSStot;          

fsfit = polyfit(fsymmvfBasket,(fsymmC66Basket./E),1);          
fsyfit = polyval(fsfit,fsymmvfBasket);          
fsSStot = sum(((fsymmC66Basket./E)-mean((fsymmC66Basket./E))).^2);                   
fsSSres = sum(((fsymmC66Basket./E)-fsyfit).^2);                       
fsRsq = 1-fsSSres/fsSStot;   

sfit = polyfit(symmvfBasket,(symmC66Basket./E),1);          
syfit = polyval(sfit,symmvfBasket);          
sSStot = sum(((symmC66Basket./E)-mean((symmC66Basket./E))).^2);                   
sSSres = sum(((symmC66Basket./E)-syfit).^2);                       
...
if sidenum == 5
    g1 = plot([svfBasket],[svbothBasket],'Marker','*',...
    'MarkerSize',6,'LineStyle','none','Color',[0.4940 0.1840 0.5560]);
elseif sidenum == 3
    g1 = plot([svfBasket],[svbothBasket],'Marker','*',...
    'MarkerSize',6,'LineStyle','none','Color',[0.4940 0.1840 0.5560]);
...
hold on
h1 = plot(fsvfBasket,fsvbothBasket,'Marker','o',...
'MarkerSize',4,'LineStyle','none','Color',[0.4660 0.6740 0.1880]);
j1 = plot(vfBasket,vbothBasket,'Marker','+',...
'MarkerSize',4,'LineStyle','none','Color',[0.3010 0.7450 0.9330]);
grid on
xlabel('Volume Fraction');
ylabel('Poisson''s Ratio');
%legend('Double-Mirror Symmetric','Mirror Symmetric','Asymmetric','Location','northeast');
axis([0 0.7 -1 1]);
set(gca,'fontsize', 13)
...
K = E/(3*(1-(2*v_c)));
G = E/(2*(1+v_c));
KHS = (4.*G.*reldens)./((4.*G)+((3.*K).*(1-reldens)));
GHS = (((9.*K)+(8.*G)).*reldens)./((20.*G)+(15.*K)+(6.*((K)+...
(2.*G)).*(1-reldens)));
EHS = (11.*GHS.*KHS)./(GHS+(3.*KHS));

j1 = plot([asymmvfBasket],[asymmCaxBasket],'Marker','+',...
'MarkerSize',4,'LineStyle','none','Color',[0.3010 0.7450 0.9330]);
hold on
h1 = plot(fsymmvfBasket,fsymmCaxBasket,'Marker','o',...
'MarkerSize',4,'LineStyle','none','Color',[0.4660 0.6740 0.1880]);
g1 = plot([symmvfBasket],[symmCaxBasket],'Marker','*',...
'MarkerSize',6,'LineStyle','none','Color',[0.4940 0.1840 0.5560]);grid on
plot(reldens,EHS,'b-','LineWidth',1);
if sidenum == 5
    plot(0.6148.*ones(1,100),0:0.01:0.99,'k--','LineWidth',1);
elseif sidenum == 3
    plot(0.5286.*ones(1,100),0:0.01:0.99,'k--','LineWidth',1);
end
xlabel('Volume Fraction');
ylabel('Normal Stiffness');
%legend('Asymmetric Designs','Mirror Symmetric Designs','Double-Mirror Symmetric Designs','Upper H-S Bound','Maximum Volume Fraction','Location','northwest');
j1.ButtonDownFcn = {@plotDesignFcn2,2}; 
h1.ButtonDownFcn = {@plotDesignFcn2,2};
g1.ButtonDownFcn = {@plotDesignFcn2,2}; 
axis([0 0.7 0 0.6]);
set(gca,'fontsize', 13)

% Develop regressions

asfit = polyfit(asymmvfBasket,asymmCaxBasket,1);          
asyfit = polyval(asfit,asymmvfBasket);          
asSStot = sum((asymmCaxBasket-mean(asymmCaxBasket)).^2);                   
asSSres = sum((asymmCaxBasket-asyfit).^2);                       
asRsq = 1-asSSres/asSStot;          

fsfit = polyfit(fsymmvfBasket,fsymmCaxBasket,1);          
fsyfit = polyval(fsfit,fsymmvfBasket);          
fsSStot = sum((fsymmCaxBasket-mean(fsymmCaxBasket)).^2);                   
fsSSres = sum((fsymmCaxBasket-fsyfit).^2);                       
fsRsq = 1-fsSSres/fsSStot;   

sfit = polyfit(symmvfBasket,symmCaxBasket,1);          
syfit = polyval(sfit,symmvfBasket);          
sSStot = sum((symmCaxBasket-mean(symmCaxBasket)).^2);                   
sSSres = sum((symmCaxBasket-syfit).^2);                       
sRsq = 1-sSSres/sSStot;
